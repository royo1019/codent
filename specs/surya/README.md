...
Specs in this folder are **owned by surya**. Ownership means surya scoped them, approved them,
reviews the unit summary blocks, and signs off "ready to ship". Claude executes; surya decides.
...
then produce a draft spec at specs/surya/<your-suggested-slug>/spec.md
...
```
The draft at specs/surya/<feature-slug>/spec.md is approved.
...
```
Read SDD_PROCESS.md. I'm surya. Find my next pending unit across
specs/surya/ and specs/_tracker.csv per §9.6. Tell me in 2–3 sentences
...
```
Read specs/_tracker.csv. I'm surya. Per §9.6, give me my queue: per
...
git checkout -b surya/<feature-slug>       # or: git checkout surya/<feature-slug> && git pull --rebase origin main
...
git commit -m "surya/<feature-slug>: <unit_id> — <one-line summary>"
git push -u origin surya/<feature-slug>
```
...
- **The LLM call budget** (`BUILD_SPEC.md §4`) still holds — no fifth unconditional call, no
  per-file fan-out.
- **`Files touched` collisions** with royo's in-flight specs, especially on the three shared paths:
